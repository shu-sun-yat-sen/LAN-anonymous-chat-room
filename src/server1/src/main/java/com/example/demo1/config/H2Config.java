...
public class H2Config implements CommandLineRunner {
    ...
    public void run(String... args) throws Exception {
        try (Connection connection = DriverManager.getConnection(dataSourceUrl, dataSourceUsername, dataSourcePassword);
            ...
            statement.execute("CREATE TABLE IF NOT EXISTS ROOM ( ROOM_NAME VARCHAR(255) PRIMARY KEY, ROOM_OWNER_ID VARCHAR(255),NUMOFPEOPLE INT,PASS_WORD VARCHAR(255),MEMBERS_ID VARCHAR(255))");
            statement.execute("CREATE TABLE IF NOT EXISTS TALK ( TIME VARCHAR(255) PRIMARY KEY, CHATROOMNAME VARCHAR(255),SENDERNAME VARCHAR(255),SENDERPIC VARCHAR(255),CONTEXT VARCHAR(255))");
